...
from app.sports.mlb.models import GameSnapshot
...
async def build_game_snapshot(team: str) -> GameSnapshot:
    ...
    try:
        ...
        snapshot = await _build_snapshot_from_game(selected_game=selected_game)
...
def _select_requested_team_game(games: list[dict], team: str) -> dict | None:
    ...
    todays_games = [game for game in matching_games if _extract_game_date_et(game) == today_et]
    if todays_games:
        logger.debug(
...
async def _build_snapshot_from_game(selected_game: dict) -> GameSnapshot:
    '''
    ...
    game_date = _parse_game_datetime(selected_game.get('gameDate'))
